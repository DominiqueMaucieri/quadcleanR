...
LQuad_long <- LQuad_cover %>% select(-c(unuseable)) %>% pivot_longer(cols = names(LQuad_useable[,4:131]), names_to = "Species", values_to = "prop_cover")

```
 
One thing you may notice by looking at the `Species` column, is that these species names are not unique species, but there are duplicates of the same species, categorized into bleaching and non bleaching forms. For any kind of diversity analysis, this would inflate the number of different species, so it is important to combine different forms of the same species if diversity analyses are being done.

For this clean up, we will walk through 3 ways of dealing with this based on what you want to accomplish.

1. Combining rows.

If you want to use your data in this long format and want to just combine everything with 



This will take a few minutes (about 6 minutes for ma) as there are over 500000 rows.

```{r categorizing bleaching}

# LQuad_Bleach <- categorize(LQuad_long, "Species", values = c("Bleach"), name = "Bleached", binary = TRUE, exact = FALSE)

```

```{r categorizing taxonomy}

# LQuad_Taxa <- categorize(LQuad_long, "Species", values = coral_labelset$full_name, name = "Taxonomic_Name", binary = FALSE, categories = coral_labelset$taxonomic_name)
