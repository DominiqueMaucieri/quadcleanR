...
First lets load the package, the dplyr package, tidyr package and the data, plus a few extras used to create thsi vignette.
...
Alright, now we can join these data frames together, and continue with one large data set. As both data sets used the same label set of names, This is easy with the `smartbind()` function out of the `gtools` package

```{r bind quad dfs}

AllQuads <- smartbind(SQuad_usable, LQuad_usable, fill = 0) %>% select(-c(unuseable))
...
visualize_app(data = AllQuads_cropclean, independent = colnames(AllQuads_cropclean[,1:13]), dependent = "prop_cover")
